"""Shared fixture builders for `test_pre_draft_snapshot.py`.

Unlike `conftest.feature_store_config` (a single fixed two-match Step 3A
fixture), Step 3B temporal-correctness tests each need their own
deliberately-shaped set of matches/rosters/timestamps. These helpers
build real canonical Parquet files (via the actual Step 2 transform
functions, never a hand-rolled approximation) from a flexible list of
match/roster specs, exactly like `conftest.py` does for its own fixed
fixture.
"""
...
def match_row(
    ...
    *,
    ...
    """One `matches` row spec, defaulting every field Step 3B does not
...
def player_rows(
    ...
    *,
    ...
    slot (0-4), not Dota position 1-5. PRE_DRAFT snapshot SQL does not
    read it.
    """
...
def build_feature_store_config(
    ...
    *,
    ...
    under `tmp_path` and return the `FeatureStoreConfig` pointing at it.

    `draft_events.parquet` is deliberately built empty: Step 3B's
    PRE_DRAFT snapshot never reads `draft_events` (see
    `test_pre_draft_snapshot_never_reads_draft_events`), so no test
    needs a real draft to exercise this module.
    """
