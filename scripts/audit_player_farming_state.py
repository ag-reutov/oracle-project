"""CLI for Slice 14 leakage-safe historical player farming state.

Causal candidate B + shrunk player history only. Does not persist a
player rating, does not add columns to FEATURE_COLUMNS, does not
modify Elo, does not call STRATZ, and does not train a win model.

Usage:
    uv run python scripts/audit_player_farming_state.py
    uv run python scripts/audit_player_farming_state.py \\
        --output data/interim/slice14_player_farming_state.json
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Development-only Slice 14 player farming-state diagnostics. "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/interim/slice14_player_farming_state.json"),
    ...
    print("=== Slice 14 leakage-safe historical player farming state ===")
    print(
        "Causal candidate B only. Residualizer coefficients and z-scale "
        "use start_time < T. Player state uses the same player's prior B "
        "with start_time < M.start_time. Shrinkage is toward zero and is "
        "chosen by predicting the next causal B, not match outcomes. "
    ...
    print(
        ...
        f"frozen_k_constant={FROZEN_SHRINKAGE_K:g}  "
    ...
    _print_table("Residualizer warm-up", report.residualizer_warmup)
    ...
    _print_table("History-size buckets (validation)", report.history_bucket_validation)
    _print_table("State distribution", report.state_distribution)
    _print_table("Persistence (prior state vs next B)", report.persistence)
    ...
    args.output.write_text(
        json.dumps(slice14_report_to_jsonable(report), indent=2) + "\n",
        encoding="utf-8",
    )
    print()
