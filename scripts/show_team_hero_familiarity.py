"""Inspect team × hero historical familiarity immediately before a match.

Developer-facing only. Reads the canonical Parquet dataset and prints each
team's currently drafted heroes plus leakage-safe historical team-hero
familiarity. Does not write files, does not change training features, and
is not a production API.

Usage:
    uv run python scripts/show_team_hero_familiarity.py --match-id 1234567890
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Show each team's drafted heroes and historical team-hero "
            "familiarity immediately before a canonical match."
        )
    )
    parser.add_argument(
        "--match-id",
        type=int,
        required=True,
        help="Canonical match_id whose team×hero history to inspect.",
    )
    return parser
...
def _display_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if "days_since_team_played_hero" in display.columns:
        display["days_since_team_played_hero"] = display[
            "days_since_team_played_hero"
        ].map(_format_days)
    return display
...
def main(argv: list[str] | None = None) -> int:
    ...
    with connect(config) as store:
        ...
        if match_rows.empty:
            ...
            return 1
        frame = build_team_hero(store, match_id=args.match_id).to_frame()

    info = match_rows.iloc[0]
    n_zero_hero = int((frame["team_prior_games_with_hero"] == 0).sum())
    n_null_rate = int(frame["team_prior_win_rate_with_hero"].isna().sum())
    n_null_share = int(frame["team_hero_share"].isna().sum())
    n_null_days = int(frame["days_since_team_played_hero"].isna().sum())

    print("=== Team × hero familiarity immediately before match ===")
    print(f"match_id: {int(info['match_id'])}")
    print(f"start_time: {info['start_time']}")
    print(f"game_version_id: {info['game_version_id']}")
    print(f"team×hero rows: {len(frame)}")
    print(f"heroes with zero prior team games: {n_zero_hero}")
    print(f"NULL team_prior_win_rate_with_hero (zero hero games): {n_null_rate}")
    print(f"NULL team_hero_share (zero team games): {n_null_share}")
    print(f"NULL days_since_team_played_hero (never played this hero): {n_null_days}")
    print(
        "historical team×hero facts → aggregated over past matches only "
        "→ current-draft familiarity for this match"
    )
    print("history is keyed by team_id; roster changes do not reset it")
    print()
