"""CLI for Slice 20: walk-forward evaluation of frozen player combat vs Elo.

Compares logistic_elo_only with logistic_elo_plus_player_combat on the
existing expanding-window development OOS folds. Does not redesign the
combat feature, does not score the frozen holdout, does not call STRATZ,
and does not change FEATURE_COLUMNS.

Usage:
    uv run python scripts/run_slice20_player_combat_benchmark.py
    uv run python scripts/run_slice20_player_combat_benchmark.py \\
        --output data/interim/slice20_player_combat_benchmark.json
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Slice 20 walk-forward: frozen player combat vs logistic Elo. "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/interim/slice20_player_combat_benchmark.json"),
    ...
    with connect(config) as store:
        ...
        report = run_slice20_player_combat_benchmark(store)

    pooled = report.pooled.iloc[0]
    print("=== Slice 20: frozen player combat vs logistic Elo ===")
    print(
        "Reference logistic_elo_only vs candidate "
        "logistic_elo_plus_player_combat "
        f"(elo block + {report.integrity['candidate_columns'][-1]}). "
        "Frozen Slice 18 candidate C, k=20, five-player arithmetic mean, "
    ...
    print()
    print(
        "Pearson corr(mean_combat_shrunk_c_diff, team_elo_delta) = "
        f"{report.combat_elo_pearson:.8f}"
    )
    print(
        "Spearman corr(mean_combat_shrunk_c_diff, team_elo_delta) = "
        f"{report.combat_elo_spearman:.8f}"
    )
    print(
        "Pearson corr(mean_combat_shrunk_c_diff, mean_farming_shrunk_b_diff) = "
        f"{report.combat_farming_pearson:.8f}"
    )
    print(
        "Spearman corr(mean_combat_shrunk_c_diff, mean_farming_shrunk_b_diff) = "
        f"{report.combat_farming_spearman:.8f}"
    )
    _print_table("Combat-diff distribution (OOS)", report.combat_distribution)
    _print_table(
        "Mean Elo diff by |combat| quantile bucket",
        report.elo_by_combat_bucket,
    )
    _print_mapping(
        "Prediction movement |p_candidate - p_reference|", report.prediction_movement
    )
    _print_table(
        "Paired log loss by |combat diff| quantile bucket",
        report.magnitude_buckets,
    )
    _print_table(
        "Paired log loss by signed combat-diff bucket",
        report.directional_buckets,
    )
    print()
    ...
    _print_mapping(
        "Qualitative comparison with frozen Slice 16 farming", report.slice16_comparison
    )
    print()
    print("--- Integrity ---")
    ...
    args.output.write_text(
        json.dumps(slice20_report_to_jsonable(report), indent=2) + "\n",
        encoding="utf-8",
    )
    print()
